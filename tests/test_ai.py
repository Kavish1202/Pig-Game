...
def test_smart_opponent_hot_forces_min_threshold_22():
    # points_needed = 20 -> base threshold = (20//2)+8 = 18
    # opponent close to winning: opp_needed <= 15 -> threshold = max(18, 22) = 22
    g = _setup(Game(), me_score=80, opp_score=90, turn_points=21, current_index=0, target=100)
    bot = SmartStrategy()
    assert bot.decide(g) == "hold"
    g.turn_points = 22
    assert bot.decide(g) == "hold"
...
def test_smart_big_gap_increases_threshold_beyond_max_cap():
    ...
    # So with t=29 it should still roll; at t=34 it should hold.
